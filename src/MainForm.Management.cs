...
public sealed partial class MainForm
{
    ...
    async Task ExportDiagnosticAsync()
    {
        ...
        try
        {
            var text = await Task.Run(() => GameManagement.Diagnostic(exe));
            ...
            if (dialog.ShowDialog(this) == DialogResult.OK) { await File.WriteAllTextAsync(dialog.FileName, text); status.Text = "诊断已保存；原始日志和个人路径未导出。"; }
        }
        catch (Exception e) { status.Text = e.Message; }
        finally { busy = false; libraryPage.Enabled = true; UpdateButtons(); }
    }
